...
function addColors() {
    ...
    colorsBox.innerHTML = lightcolors.map(color => {
        return `<div class="color${color.color === activeColor ? " activeColor" : ""}" style="background: ${color.color}" value="${color.color}" darkColor="${color.darkColor}" imageHue="${color.imageHue}"></div>`;
    }).join("");
    ...
    colors.forEach(color => {
        color.addEventListener('click', function() {
            if(color.classList.contains('activeColor')) {
                ...
                const activeColorBox = colorpickerBox.querySelector('.activeColor');
